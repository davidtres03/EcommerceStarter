    ...
    private async Task<OperationResult> RegisterInWindowsAsync(InstallationConfig config)
    {
        try
        {
            ...
            var script = $@"
                ...
                # Store database connection details for upgrades
                Set-ItemProperty -Path $regPath -Name 'DatabaseServer' -Value '{config.DatabaseServer}';
                Set-ItemProperty -Path $regPath -Name 'DatabaseName' -Value '{config.DatabaseName}';

                Write-Output 'Registered in Windows successfully with Change support and database details';
            ";
