...
**Purpose:** Comprehensive checklist for upgrading EcommerceStarter Supply Co. to EcommerceStarter latest version
...
- [ ] Test-Migration.ps1 run successfully
  ...
  .\Test-Migration.ps1 -SourceDatabase "EcommerceStarter"
  ```
- [ ] All 9 tests passed ?
...
- [ ] App pool running
  ```powershell
  Get-WebAppPoolState -Name "ecommercestarter"
  ```

### **Database Status:**
- [ ] Database accessible
  ```powershell
  sqlcmd -S localhost\SQLEXPRESS -E -Q "SELECT DB_ID('EcommerceStarter')" -h-1
  ```
...
- [ ] Database size noted (for comparison after)
  ...
  WHERE database_id = DB_ID('EcommerceStarter')
  ```
- [ ] Last backup timestamp confirmed
  ...
  WHERE database_name = 'EcommerceStarter'
...
- [ ] Backup started
  ...
  $backupPath = "C:\Backups\PreUpgrade\ecommercestarter_$timestamp.bak"
  $query = "BACKUP DATABASE [EcommerceStarter] TO DISK = N'$backupPath' WITH COMPRESSION, INIT, FORMAT;"
...
#### **Phase 4: Test Blue**
- [ ] Homepage loads (https://EcommerceStarter.com:8443/)
...
- [ ] Port 443 removed from Green
  ```powershell
  Remove-WebBinding -Name "EcommerceStarter" -Protocol https -Port 443
  ```
- [ ] Port 443 added to Blue
  ```powershell
  New-WebBinding -Name "EcommerceStarter-Blue" -Protocol https -Port 443
  ```
- [ ] SSL certificate bound to Blue:443
- [ ] Switch timestamp noted: ________________

#### **Phase 6: Monitor Blue**
- [ ] Site accessible on https://EcommerceStarter.com/ (port 443)
...
- [ ] App pool running stably
  ```powershell
  Get-WebAppPoolState -Name "ecommercestarter-Blue" # or "ecommercestarter"
  ```
...
Remove-WebBinding -Name "EcommerceStarter-Blue" -Protocol https -Port 443
New-WebBinding -Name "EcommerceStarter" -Protocol https -Port 443
...
Rename-Item "C:\inetpub\ecommercestarter" "C:\inetpub\ecommercestarter-failed"
Rename-Item "C:\inetpub\ecommercestarter-old" "C:\inetpub\ecommercestarter"
```

**Database Rollback (If Schema Changed):**
```powershell
$backupFile = "C:\Backups\PreUpgrade\ecommercestarter_20251109_123456.bak"
...
- **Code Location:** `C:\inetpub\ecommercestarter\`
- **Database Name:** `EcommerceStarter`
- **Production URL:** https://EcommerceStarter.com/
