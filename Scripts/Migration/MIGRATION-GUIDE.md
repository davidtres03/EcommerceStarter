...
**Purpose:** Safely migrate production database (EcommerceStarter Supply Co.) to work with EcommerceStarter open-source code.
...
```
Production Database (EcommerceStarter)
          ?
    [Backup & Restore]
          ?
...
.\Test-Migration.ps1 `
    -SourceDatabase "EcommerceStarter" `
...
TO DISK = N'C:\Temp\DatabaseBackups\EcommerceStarter_Backup.bak' 
WITH FORMAT, INIT, COMPRESSION;
```

**Or PowerShell:**
```powershell
$backupPath = "C:\Temp\DatabaseBackups\EcommerceStarter_$(Get-Date -Format 'yyyyMMdd_HHmmss').bak"
$query = "BACKUP DATABASE [EcommerceStarter] TO DISK = N'$backupPath' WITH FORMAT, INIT, COMPRESSION;"
...
FROM DISK = N'C:\Temp\DatabaseBackups\EcommerceStarter_Backup.bak' 
WITH MOVE N'EcommerceStarter' TO N'C:\...\EcommerceStarter_Test.mdf',
     MOVE N'EcommerceStarter_log' TO N'C:\...\EcommerceStarter_Test_log.ldf',
     REPLACE;
```
...
- [ ] Branding shows correctly (EcommerceStarter ??)
- [ ] Logo displays
...
VALUES ('EcommerceStarter Supply Co.', 'support@ecommercestarter.com', 'orange');
```
...
2. **Merge to Production Branch:**
   - Keep separate branches:
     ...
     - `production` ? ecommercestarter specific code
...
### Application Code
- [ ] No hardcoded ecommercestarter references
...
-NoProfile -ExecutionPolicy Bypass -Command "& {
    ...
    `$backup = 'C:\Temp\DatabaseBackups\EcommerceStarter_`$date.bak'
    Invoke-Sqlcmd -Query \"BACKUP DATABASE [EcommerceStarter] TO DISK = N'`$backup' WITH COMPRESSION\"
}"
"@

$trigger = New-ScheduledTaskTrigger -Daily -At 2am

Register-ScheduledTask -TaskName "Backup ecommercestarter DB" -Action $action -Trigger $trigger -RunLevel Highest
```
...
```powershell
.\Test-Migration.ps1 -SkipBackup:$false -SourceDatabase "EcommerceStarter"
```
...
dotnet publish -c Release -o C:\inetpub\ecommercestarter
```

4. **Run Migrations (if needed)**
```bash
dotnet ef database update --connection "Server=...;Database=EcommerceStarter;..."
```
...
6. **Verify**
- Check https://EcommerceStarter.com/
- Test critical paths
- Monitor logs
...
$backup = "C:\Temp\DatabaseBackups\EcommerceStarter_PreDeployment.bak"
Invoke-Sqlcmd -Query "RESTORE DATABASE [EcommerceStarter] FROM DISK = N'$backup' WITH REPLACE;"
...
? **Backed up EcommerceStarter database**
? **Restored to test environment**
? **Tested with EcommerceStarter code**
? **Verified:**
   - Branding preserved (EcommerceStarter ??)
   - Products preserved
   - Users preserved
   - Admin login works
   - Theme colors correct (orange)
...
**Tested:** ? Successfully validated with EcommerceStarter database
