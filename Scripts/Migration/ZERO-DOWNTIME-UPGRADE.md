...
**Goal:** Upgrade EcommerceStarter Supply Co. production site with ZERO customer-facing downtime
...
**Your site (https://EcommerceStarter.com/):**
...
4. **Code Compatibility** ?
   ...
   - EcommerceStarter database + EcommerceStarter code = SUCCESS
   - All data preserved
...
```
BEFORE:
C:\inetpub\ecommercestarter\        ? IIS serves from here (Version 1.0)
C:\inetpub\ecommercestarter-new\    ? New version staged (Version 1.1)

SWAP (< 1 second):
Rename ecommercestarter ? ecommercestarter-old
Rename ecommercestarter-new ? ecommercestarter

AFTER:
C:\inetpub\ecommercestarter\        ? IIS serves from here (Version 1.1) ?
C:\inetpub\ecommercestarter-old\    ? Old version (rollback ready)
```
...
cd C:\inetpub\ecommercestarter-new
dotnet ef database update

# 3. Atomic directory swap (< 1 second downtime)
$old = "C:\inetpub\ecommercestarter"
$new = "C:\inetpub\ecommercestarter-new"
$backup = "C:\inetpub\ecommercestarter-old"
...
1. **Deploy new version** (separate port or subdomain)
   - https://EcommerceStarter.com:8080 (new)
   - https://EcommerceStarter.com (old, still live)
...
## ?? Recommended Approach for EcommerceStarter
...
#    - IIS Site: "EcommerceStarter" (port 443)
#    - App Pool: "ecommercestarter"
#    - Path: C:\inetpub\ecommercestarter
#    - Database: EcommerceStarter

# 2. Create Blue (new) environment
New-WebAppPool -Name "ecommercestarter-Blue"
New-WebSite -Name "EcommerceStarter-Blue" `
            -Port 8443 `
            -PhysicalPath "C:\inetpub\ecommercestarter-blue" `
            -ApplicationPool "ecommercestarter-Blue"

# 3. Deploy new code to Blue
Copy-Item "C:\Temp\UpgradePackage\*" `
          "C:\inetpub\ecommercestarter-blue\" `
          -Recurse
...
#    https://EcommerceStarter.com:8443/
#    Verify everything works
...
Remove-WebSite -Name "EcommerceStarter" # Old green site
Remove-WebAppPool -Name "ecommercestarter"      # Old app pool
```
...
$query = "BACKUP DATABASE [EcommerceStarter] TO DISK = N'$backupPath' WITH COMPRESSION;"
sqlcmd -S localhost\SQLEXPRESS -E -Q $query

# Code backup
Copy-Item "C:\inetpub\ecommercestarter" `
...
Set-ItemProperty "IIS:\AppPools\ecommercestarter-Blue" -Name "managedRuntimeVersion" -Value "v4.0"
Set-ItemProperty "IIS:\AppPools\ecommercestarter-Blue" -Name "enable32BitAppOnWin64" -Value $false
Set-ItemProperty "IIS:\AppPools\ecommercestarter-Blue" -Name "processModel.identityType" -Value "ApplicationPoolIdentity"

# Start app pool
Start-WebAppPool -Name "ecommercestarter-Blue"
```
...
New-Item -Path "C:\inetpub\ecommercestarter-blue" -ItemType Directory -Force

# Create IIS site
New-WebSite -Name "EcommerceStarter-Blue" `
            -Port 8443 `
            -HostHeader "EcommerceStarter.com" `
            -PhysicalPath "C:\inetpub\ecommercestarter-blue" `
            -ApplicationPool "ecommercestarter-Blue" `
            -Ssl
...
$cert = Get-ChildItem -Path "Cert:\LocalMachine\My" | 
        Where-Object { $_.Subject -like "*EcommerceStarter.com*" }

New-WebBinding -Name "EcommerceStarter-Blue" `
               -Protocol https `
               -Port 8443 `
               -HostHeader "EcommerceStarter.com" `
               -SslFlags 0
...
# Bind certificate
$binding = Get-WebBinding -Name "EcommerceStarter-Blue" -Protocol https
...
Copy-Item "C:\inetpub\ecommercestarter\appsettings.Production.json" `
          "C:\inetpub\ecommercestarter-blue\appsettings.Production.json"

# Set permissions
$acl = Get-Acl "C:\inetpub\ecommercestarter-blue"
$identity = "IIS AppPool\ecommercestarter-Blue"
$permission = $identity, "Read,Execute", "ContainerInherit,ObjectInherit", "None", "Allow"
$rule = New-Object System.Security.AccessControl.FileSystemAccessRule $permission
$acl.AddAccessRule($rule)
Set-Acl "C:\inetpub\ecommercestarter-blue" $acl
```

#### **Step 4: Run Migrations on Blue**

```powershell
cd C:\inetpub\ecommercestarter-blue
...
```powershell
# Test homepage
Invoke-WebRequest -Uri "https://EcommerceStarter.com:8443/" `
                  -UseBasicParsing

# Test admin login
Start-Process "https://EcommerceStarter.com:8443/Admin/Dashboard"
```
...
Get-Website -Name "EcommerceStarter-Blue" | Select-Object Name, State

# 2. Verify Green is still running (fallback)
Get-Website -Name "EcommerceStarter" | Select-Object Name, State

# 3. Note current traffic (for comparison)
Get-Counter -Counter "\Web Service(EcommerceStarter)\Current Connections"
```
...
Remove-WebBinding -Name "EcommerceStarter" `
                  -Protocol https `
                  -Port 443
...
$cert = Get-ChildItem -Path "Cert:\LocalMachine\My" | 
        Where-Object { $_.Subject -like "*EcommerceStarter.com*" }
$binding = Get-WebBinding -Name "EcommerceStarter-Blue" -Protocol https -Port 443
$binding.AddSslCertificate($cert.Thumbprint, "My")
...
# 3. Monitor app pool
Get-WebAppPoolState -Name "ecommercestarter-Blue"

# 4. Check active connections
Get-Counter -Counter "\Web Service(EcommerceStarter-Blue)\Current Connections"
...
Remove-WebBinding -Name "EcommerceStarter-Blue" `
                  -Protocol https `
                  -Port 443

# Step 2: Add port 443 back to Green
New-WebBinding -Name "EcommerceStarter" `
               -Protocol https `
               -Port 443 `
               -HostHeader "EcommerceStarter.com"
...
$cert = Get-ChildItem -Path "Cert:\LocalMachine\My" | 
        Where-Object { $_.Subject -like "*EcommerceStarter.com*" }
$binding = Get-WebBinding -Name "EcommerceStarter" -Protocol https -Port 443
$binding.AddSslCertificate($cert.Thumbprint, "My")
...
# Then delete
Remove-WebSite -Name "EcommerceStarter" -Confirm
Remove-WebAppPool -Name "ecommercestarter" -Confirm
Remove-Item "C:\inetpub\ecommercestarter" -Recurse -Confirm
```
