...
**Daily development workflow for keeping EcommerceStarter open-source while actively developing EcommerceStarter production.**
...
Invoke-Sqlcmd -ServerInstance "localhost\SQLEXPRESS" -Query "BACKUP DATABASE [EcommerceStarter] TO DISK = N'$backup' WITH COMPRESSION"
```
...
- Hardcode EcommerceStarter specific values
...
3. **If tests pass ? Deploy to production**
   ...
   # Restart IIS
   Restart-WebAppPool -Name "ecommercestarter"
   ```
...
```powershell
# 1. Backup production
.\Test-Migration.ps1 -SourceDatabase "EcommerceStarter" -BackupPath "C:\Backups\PreDeployment"
...
# 6. Smoke test
Invoke-WebRequest https://EcommerceStarter.com/ -UseBasicParsing
```
...
```powershell
# 1. Monitor logs
Get-Content C:\inetpub\ecommercestarter\logs\*.log -Tail 50 -Wait
...
```powershell
# Check site status
Get-Website | Where-Object { $_.Name -like "*ecommercestarter*" }

# Restart app pool
Restart-WebAppPool -Name "ecommercestarter"

# Restart website
Restart-WebSite -Name "EcommerceStarter"
...
RESTORE DATABASE [EcommerceStarter] FROM DISK = N'$($backup.FullName)' WITH REPLACE;
