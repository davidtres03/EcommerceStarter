...
### **Scenario: Upgrade Cap & Collar Production**

```
Current State:
- Site: https://capandcollarsupplyco.com/ (LIVE)
- Database: CapAndCollarSupplyCo (SQL Server)
...
- [ ] **Backup production database**
  ...
  .\Test-Migration.ps1 -SourceDatabase "CapAndCollarSupplyCo" -BackupPath "C:\Backups\PreUpgrade"
  ```
...
3. **Configuration Screen:**
   - **Database:**
     ...
     - Database: `CapAndCollarSupplyCo` (EXISTING!)
     - Test Connection ?
...
# Check admin login
Start-Process "https://capandcollarsupplyco.com/Admin/Dashboard"
```
...
```powershell
# Start app pool
Start-WebAppPool -Name "CapAndCollar"

# Start site
Start-WebSite -Name "CapAndCollarSupplyCo"

# Remove maintenance page (if used)
Remove-Item "C:\inetpub\capandcollar\maintenance.html" -ErrorAction SilentlyContinue
```
...
Get-ChildItem "C:\inetpub\capandcollar\logs\" -Filter "*.log" | 
    Get-Content -Tail 50
```
...
$oldPath = "C:\inetpub\capandcollar"
$newPath = "C:\inetpub\capandcollar-new"
$tempPath = "C:\inetpub\capandcollar-old"
...
sqlcmd -S localhost\SQLEXPRESS -d CapAndCollarSupplyCo -i migration.sql
...
VALUES ('Cap & Collar Supply Co.', 'orange');
```
