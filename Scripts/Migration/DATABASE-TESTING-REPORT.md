...
sqlcmd -S "localhost\SQLEXPRESS" -E -Q "SELECT name FROM sys.databases WHERE name IN ('EcommerceStarter', 'EcommerceStarter_Test')" -h-1

# Result: EcommerceStarter
```
...
1. Prerequisites Check
   ...
   ?? Source database exists ? (confirmed EcommerceStarter exists)
...
```powershell
sqlcmd -S localhost\SQLEXPRESS -E -Q "SELECT DB_ID('EcommerceStarter') as DbId" -h-1
# Expected: (a number) - confirms database exists
```

**Test 2: Check Tables**
```powershell
sqlcmd -S localhost\SQLEXPRESS -E -d EcommerceStarter -Q "SELECT COUNT(*) FROM Products" -h-1
sqlcmd -S localhost\SQLEXPRESS -E -d EcommerceStarter -Q "SELECT COUNT(*) FROM AspNetUsers" -h-1
sqlcmd -S localhost\SQLEXPRESS -E -d EcommerceStarter -Q "SELECT COUNT(*) FROM Orders" -h-1
# Expected: Row counts
```

**Test 3: Check Settings**
```powershell
sqlcmd -S localhost\SQLEXPRESS -E -d EcommerceStarter -Q "SELECT CompanyName FROM Settings" -h-1
# Expected: "EcommerceStarter Supply Co."
```
