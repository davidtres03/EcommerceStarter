...
public partial class MaintenanceModePage : Page
{
    ...
    private async void Upgrade_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            // Find the ZIP asset
            var zipAsset = latestRelease.FindAssetByPattern("EcommerceStarter-Installer-*.zip");
            if (zipAsset == null)
            {
                MessageBox.Show(
            ...
            // Find the upgrader in the extracted package
            var upgraderPath = Path.Combine(tempFolder, "Installer", "EcommerceStarter.Upgrader.exe");

            if (!File.Exists(upgraderPath))
            {
                MessageBox.Show(
                    $"Upgrader not found in package at:\n{upgraderPath}\n\n" +
