...
public class UpgradeService
{
    ...
    public async Task<UpgradeResult> UpgradeFromZipAsync(ExistingInstallation installation, string zipFilePath, string? newVersion = null)
    {
        ...
        try
        {
            ...
            // NOTE: Step 5b removed - upgrade flow now uses external EcommerceStarter.Upgrader.exe
            // The installer's maintenance mode launches the upgrader, which handles the full upgrade
            // This UpgradeService class is kept for legacy compatibility only
    ...
    private async Task LaunchUpgraderAsync(string tempExtractPath)
    {
        await Task.Run(() =>
        {
            try
            {
                // Find the upgrader executable in the package
                var upgraderPath = Path.Combine(tempExtractPath, "Installer", "EcommerceStarter.Upgrader.exe");
                
                if (!File.Exists(upgraderPath))
                {
                    StatusUpdate?.Invoke(this, $"[LaunchUpgraderAsync] WARNING: Upgrader not found at {upgraderPath}");
                    StatusUpdate?.Invoke(this, "[LaunchUpgraderAsync] Installer update will be skipped");
                    return;
                }

                // Get current process ID so upgrader can wait for us to exit
                var currentPid = Process.GetCurrentProcess().Id;

                // Get Program Files path where installer lives
                var programFilesPath = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
                var installerDir = Path.Combine(programFilesPath, "EcommerceStarter");

                // Launch upgrader in background
                // Args: <installerPID> <packagePath> <programFilesPath>
                var startInfo = new ProcessStartInfo
                {
                    FileName = upgraderPath,
                    Arguments = $"{currentPid} \"{tempExtractPath}\" \"{installerDir}\"",
                    UseShellExecute = true, // Run detached
                ...
                if (process != null)
                {
                    StatusUpdate?.Invoke(this, $"[LaunchUpgraderAsync] ✓ Upgrader launched (PID: {process.Id})");
                    StatusUpdate?.Invoke(this, "[LaunchUpgraderAsync] Upgrader will update Program Files installer after this process exits");
                }
                else
                {
                    StatusUpdate?.Invoke(this, "[LaunchUpgraderAsync] ERROR: Failed to start upgrader process");
                }
            }
            catch (Exception ex)
            {
                StatusUpdate?.Invoke(this, $"[LaunchUpgraderAsync] ERROR: {ex.Message}");
            }
        });
    }
        ...
        try
        {
            ...
            var script = $@"
                ...
                try {{
                    ...
                    $checkColumnCmd.CommandText = ""SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'SiteSettings' AND COLUMN_NAME = 'InternalServiceKeyEncrypted'"";
                    ...
                    Write-Output ""Generated new internal service key for automated testing"";

                    $updateCmd = $connection.CreateCommand();
                    $updateCmd.CommandText = 'UPDATE SiteSettings SET InternalServiceKeyEncrypted = @Key, EnableInternalServiceAuth = 1 WHERE Id = 1';
                    $updateCmd.Parameters.AddWithValue('@Key', $newKey);
                    $updateCmd.ExecuteNonQuery();

                    Write-Output 'Internal service key configured (will be encrypted by application)';
                    $connection.Close();
                }}
                catch {{
