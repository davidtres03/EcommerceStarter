...
public class UpgradeService
{
    ...
    private async Task<OperationResult> RunMigrationsAsync(string installPath, string server, string database)
    {
        try
        {
            ...
            // Navigate up from install path to find project file
            var currentDir = Directory.GetParent(installPath);
            if (currentDir == null) return new OperationResult { Success = false, Message = "Invalid install path" };
            
            currentDir = Directory.GetParent(currentDir.FullName);
            if (currentDir == null) return new OperationResult { Success = false, Message = "Invalid directory structure" };
            ...
            var projectPath = Path.Combine(currentDir.FullName, "EcommerceStarter", "EcommerceStarter.csproj");
