...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$Version = $null  # Will auto-detect from csproj if not provided
)
...
$UpgraderProject = Join-Path $ProjectRoot "EcommerceStarter.Upgrader\EcommerceStarter.Upgrader.csproj"

# Auto-detect version from csproj if not provided
if ([string]::IsNullOrEmpty($Version)) {
    $csprojContent = Get-Content $InstallerProject -Raw
...
$UpgraderBuildDir = Join-Path $TempBuildDir "upgrader"
...
if (-not (Test-Path $UpgraderProject)) {
    Write-Error-Custom "Upgrader project not found: $UpgraderProject"
    exit 1
}
...
New-Item -ItemType Directory -Path $UpgraderBuildDir -Force | Out-Null
...
dotnet ef migrations bundle `
    ...
    --force 2>&1 | Select-Object -Last 5
...
# Copy logging helper script
Write-Info "Copying logging helper script..."
$loggingScript = Join-Path (Split-Path $InstallerProject -Parent) "RunInstallerWithLogging.ps1"
if (Test-Path $loggingScript) {
    Copy-Item -Path $loggingScript -Destination $FinalPackageDir -Force
    Write-Success "Logging script copied"
}
...
# Note: Compress-Archive will overwrite existing file, no need to remove
# if (Test-Path $zipPath) {
#     Remove-Item $zipPath -Force
# }

Write-Info "Compressing package..."
# Compress from parent directory so the ZIP contains app/, migrations/, etc at root level
# instead of nested under EcommerceStarter-Installer-vX.X.X/
$ParentDir = Split-Path -Parent $FinalPackageDir
$PackageFolderName = Split-Path -Leaf $FinalPackageDir

# Delete existing ZIP using .NET since Compress-Archive has handle issues
if (Test-Path $zipPath) {
...
try {
    Compress-Archive -Path $PackageFolderName -DestinationPath $zipPath -CompressionLevel Optimal
}
finally {
    Pop-Location
}
