    ...
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostUploadImageAsync(IFormFile imageFile)
        {
            try
            {
                ...
                _logger.LogInformation("Product image uploaded: {Url}", imageUrl);
        ...
        public async Task<IActionResult> OnPostUploadProductImageAsync(IFormFile file, int productId)
        {
            try
            {
                ...
                var imageUrl = await _imageUploadService.UploadImageAsync(file, "images/products");

                _logger.LogInformation("Product image uploaded for product {ProductId}: {Url}", productId, imageUrl);

                return new JsonResult(new { success = true, message = "Image uploaded successfully!", imageUrl = imageUrl });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error uploading product image");
        ...
        public async Task<IActionResult> OnPostUploadVariantImageAsync(IFormFile file, int? variantId = null)
        {
            try
            {
                ...
                {
                    _logger.LogInformation("Variant image uploaded for variant {VariantId}: {Url}", variantId, imageUrl);
                }
                else
                {
                    _logger.LogInformation("Variant image uploaded (new variant): {Url}", imageUrl);
                }
