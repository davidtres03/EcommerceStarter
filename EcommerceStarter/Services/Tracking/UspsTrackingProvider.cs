...
namespace EcommerceStarter.Services.Tracking
{
    ...
    public class UspsTrackingProvider : ICarrierTrackingProvider
    {
        ...
        public async Task<bool> IsEnabledAsync()
        {
            ...
            return settings.UspsEnabled && 
                   !string.IsNullOrEmpty(settings.UspsUserId) && // Consumer Key
                   !string.IsNullOrEmpty(settings.UspsPasswordEncrypted); // Consumer Secret
        }
        ...
        private async Task<string?> GetAccessTokenAsync()
        {
            try
            {
                ...
                var settings = await _apiKeyService.GetApiKeySettingsAsync();
                var consumerKey = await _apiKeyService.GetUspsUserIdAsync(); // Consumer Key
                var consumerSecret = await _apiKeyService.GetUspsPasswordAsync(); // Consumer Secret
                ...
                var useSandbox = settings.UspsUseSandbox;
        ...
        private async Task<JsonElement?> CallUspsTrackingApiAsync(string trackingNumber, string accessToken)
        {
            try
            {
                ...
                var settings = await _apiKeyService.GetApiKeySettingsAsync();
                var trackingUrl = settings.UspsUseSandbox ? USPS_SANDBOX_TRACKING_URL : USPS_PRODUCTION_TRACKING_URL;
                var environment = settings.UspsUseSandbox ? "SANDBOX" : "PRODUCTION";
